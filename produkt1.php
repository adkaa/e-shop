    ...
    <div id="myCarousel" class="carousel slide" data-ride="carousel">
      ...
      <div class="carousel-inner" role="listbox">
        <div class="item active">
          <img class="d-block w-70"  src="./img/epiphyllum-anguliger-1.jpg" alt="Image">
          <div class="carousel-caption">
          </div>      
        </div>

        <div class="item">
          <img src="./img/epiphyllum-anguliger-2.jpg" alt="Image">
          <div class="carousel-caption">
          </div>      
        </div>
      </div>
...
$sql = 'select cena from produkty where id=1';
$v = mysqli_query($db,$sql);
$row = mysqli_fetch_assoc($v);
echo "<div>";
    echo "<h2>" .$row['cena'] ." € </h2>";
        $sql = 'select mnozstvo from produkty where id=1';
        $v = mysqli_query($db,$sql);
        $row = mysqli_fetch_assoc($v) ;
        if ($row['mnozstvo'] !=0){
        echo "<form role=\"form\" method=post action =\"\">";
            echo "<input type=\"hidden\" name=\"akce\" value=\"produkt1\">";
